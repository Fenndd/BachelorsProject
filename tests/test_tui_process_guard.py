...
def test_experiment_back_is_blocked_while_active(monkeypatch) -> None:
...
def test_experiment_back_is_not_blocked_when_inactive(monkeypatch) -> None:
    screen = ExperimentScreen()
    popped: list[bool] = []
...
def test_experiment_force_unlock_clears_active_state(monkeypatch) -> None:
    screen = ExperimentScreen()
    buttons = {
        "#dry-run": SimpleNamespace(disabled=True),
        "#real-run": SimpleNamespace(disabled=True),
    }
    statuses: list[str] = []
    unregistered: list[bool] = []
    ...
    monkeypatch.setattr(
        screen,
        "query_one",
        lambda selector, *args, **kwargs: buttons[selector],
    )
    monkeypatch.setattr(screen, "_set_status_message", statuses.append)
    monkeypatch.setattr(screen, "_unregister_process", lambda: unregistered.append(True))
    ...
    assert screen._state == "failed"
    assert not buttons["#dry-run"].disabled
    assert not buttons["#real-run"].disabled
    assert unregistered == [True]
    assert statuses == [
        "Status: force-unlocked. Check workspace/tui_debug.log and Task Manager for leftover python processes."
    ]
