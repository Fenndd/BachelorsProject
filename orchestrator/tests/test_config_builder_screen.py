...
def _make_screen(monkeypatch, tmp_path: Path, overrides: dict[str, object] | None = None) -> tuple[ConfigBuilderView, _FakeLog]:
    ...
    values: dict[str, object] = {
        ...
        "allowed_files": _FakeTextArea("cpp/external/lambdatwist/p3p.cc"),
        ...
        "variant_id": SimpleNamespace(value="default"),
        "variant_description": SimpleNamespace(value=""),
        "variant_llm_config": SimpleNamespace(value="configs/llm_mock.json"),
...
def test_invalid_payload_does_not_save_or_start(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        SimpleNamespace(
            ...
            push_screen=lambda screen_obj, callback=None: pushes.append(screen_obj),
        ),
    )
...
def test_reporting_disabled_writes_enabled_false(monkeypatch, tmp_path: Path) -> None:
    screen, _log = _make_screen(
        monkeypatch,
        tmp_path,
        {"reporting_html": False, "reporting_pdf": False},
    )
...
def test_baseline_filtering_excludes_failed_candidate_and_mismatched(monkeypatch, tmp_path: Path) -> None:
    ...
    nested = root / "results" / "runs" / "group" / "it_01"
    nested.mkdir(parents=True)
    (nested / "status.json").write_text('{"overall_status": "success"}\n', encoding="utf-8")

    monkeypatch.setattr(config_builder, "get_project_paths", lambda: SimpleNamespace(repo_root=root))

    options = config_builder._discover_baseline_runs("lambdatwist_p3p")
