"""Tests for the read-only final repeated-median comparison module."""
...
def _candidate_dir(tmp_path: Path, runtime: float = 800.0) -> Path:
    candidate = tmp_path / "results" / "runs" / "candidate"
    write_json(candidate / "verification.json", make_benchmark_payload(runtime))
    return candidate
...
def test_baseline_is_best_writes_skipped_report(tmp_path: Path) -> None:
    ...
    assert report["report_type"] == "repeated_median_final_selection_report"
...
def test_non_baseline_uses_existing_verification_without_eigen(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.delenv("EIGEN3_INCLUDE_DIR", raising=False)
    exp_dir = _experiment_dir(tmp_path)
    baseline = _baseline_dir(tmp_path)
    candidate = _candidate_dir(tmp_path, runtime=800.0)
    src = _final_source_dir(tmp_path)
...
def test_report_reads_existing_candidate_repeated_fields(tmp_path: Path) -> None:
    exp_dir = _experiment_dir(tmp_path)
    baseline = _baseline_dir(tmp_path, runtime=1000.0)
    candidate = _candidate_dir(tmp_path, runtime=800.0)
    src = _final_source_dir(tmp_path)
    ...
    report = json.loads(report_path.read_text(encoding="utf-8"))
    assert report["status"] == "completed"
    assert report["comparison"]["final_benchmark_run_count"] == 100
    assert report["comparison"]["decision_metric"] == "median_runtime_ns_per_problem_median"
