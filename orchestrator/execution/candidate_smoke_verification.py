"""Legacy narrow verifier for one materialized candidate smoke test.

This module has been superseded by candidate_benchmark_verification.py for the
public verify_candidate command. It remains available for compatibility with
older imports and manual smoke-only checks.
...
def _build_summary(
    ...
    lines.extend(
        [
            ...
            "This legacy smoke-only verifier does not run family benchmarks or compare performance.",
            "",
        ]
    )
    return "\n".join(lines)
