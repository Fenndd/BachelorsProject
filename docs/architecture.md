...
The current baseline path is now the benchmark-family baseline path:

1. CMake configures the C++ project.
2. The Lambda Twist baseline target and project-owned executables are built.
3. A smoke test, runner, adapter validator, and absolute-pose family benchmark execute from the Python CLI entry point.
...
- `orchestrator.experiments.run_experiment`: runs configured candidate-generation iterations with optional materialization and benchmark-family verification.
...
Still not implemented: automatic promotion of candidates into the main source tree, baseline-vs-candidate performance comparison, best candidate selection, and full reporting.
